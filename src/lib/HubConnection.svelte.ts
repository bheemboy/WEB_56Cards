...
export class HubConnector {
  ...
  // Helper to update connection state safely
  private updateConnectionState(newState: string): void {
    console.log(`Connection state changing: ${this._connectionState} -> ${newState}`);
  ...
  public async invokeMethod(methodName: string, ...args: any[]): Promise<any> {
    ...
    if (this._hubConnection.state as string !== signalR.HubConnectionState[signalR.HubConnectionState.Connected]) {
      try {
        await this.connect();

        // Add a brief delay to ensure connection is stable
        await new Promise(resolve => setTimeout(resolve, 50));

        // Re-check connection state after delay - with fixed comparison
        if (this._hubConnection.state as string !== signalR.HubConnectionState[signalR.HubConnectionState.Connected]) {
          throw new Error(`Connection still not ready after connect() call`);
        }
      } catch (error) {
